...
def getSimilarity(tweet_id, cluster_id):
    ...
    for term in set(cluster_text[cluster_id]):
        ...
        cluster_weight_vector[term] = calc_tf(term, cluster_text[cluster_id])   #*cluster_idf
...
def tf_idf_tweet():
    ...
    for term in set(current_cleanTweet):
        ...
        if cluster_freq[term]!=0:
            idf =  math.log10(float(total_clusters) / cluster_freq[term])
        else:
            idf = 0.0
        tf_idf = tf * idf
        tweet_vector[term] = tf_idf
...
for cluster_id in clusters:
    for term in cluster_text[cluster_id]:
        if cluster_freq[term] != 0.0:
            cluster_idf = math.log10(float(total_clusters) / cluster_freq[term])
        else:
            cluster_idf = 0.0
        clusterWords[cluster_id][term] = cluster_idf * calc_tf(term,cluster_text[cluster_id])
